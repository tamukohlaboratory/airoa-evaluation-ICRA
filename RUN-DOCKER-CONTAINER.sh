...
export POLICY_SERVER_HOST=172.20.10.3
...
cmd_shell() {
  ...
  if [[ -n "${cyclonedds_interface}" || -n "${cyclonedds_interface_address}" ]]; then
    ...
    local socket_receive_buffer_block=""
    if [[ -n "${CYCLONEDDS_SOCKET_RECEIVE_BUFFER_SIZE:-}" ]]; then
      socket_receive_buffer_block="      <SocketReceiveBufferSize min=\"${CYCLONEDDS_SOCKET_RECEIVE_BUFFER_SIZE}\"/>"
    fi
