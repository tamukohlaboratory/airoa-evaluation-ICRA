...
class LocalLeRobotDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.load_camera_data = load_camera_data
        ...
        logging.info(
            "LocalLeRobotDataset ready: root=%s selected_episodes=%d total_frames=%d video_keys=%s image_keys=%s load_camera_data=%s",
            self.root,
            self.num_episodes,
            self.num_frames,
    ...
    def episode_lengths(self) -> np.ndarray:
        return (self._selected_to - self._selected_from).copy()
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if self.load_camera_data and len(self.meta.video_keys) > 0:
        ...
        elif not self.load_camera_data:
            for cam_key in self.meta.camera_keys:
                item.setdefault(cam_key, None)

        if self.load_camera_data and self.image_transforms is not None:
            for cam_key in self.meta.camera_keys:
                if cam_key in item:
