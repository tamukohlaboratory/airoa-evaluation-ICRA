...
@dataclasses.dataclass(frozen=True)
class HSRInputs(transforms.DataTransformFn):
    """Inputs for the Toyota HSR policy.
    ...
    - state: [8]
      [arm_lift_joint, arm_flex_joint, arm_roll_joint, wrist_flex_joint,
       wrist_roll_joint, gripper, head_pan_joint, head_tilt_joint]
    - actions: [action_horizon, 11]
      [arm_lift_joint, arm_flex_joint, arm_roll_joint, wrist_flex_joint,
       wrist_roll_joint, gripper, head_pan_joint, head_tilt_joint,
    ...
    def __call__(self, data: dict) -> dict:
        ...
        inputs = {
            "state": state,
            "image": {
                # HSR does not have a base camera; keep the channel but mask it out.
                "base_0_rgb": np.zeros_like(data["hand_rgb"]),
                "left_wrist_0_rgb": data["hand_rgb"],
                "right_wrist_0_rgb": data["head_rgb"],
            },
            "image_mask": {
...
def _gripper_to_angular(value):
    value = _unnormalize(value, min_val=0, max_val=0.032) + 0.01844

    def linear_to_radian(linear_position, arm_length, horn_radius):
        inner = (horn_radius**2 + linear_position**2 - arm_length**2) / (2 * horn_radius * linear_position)
        return np.arcsin(np.clip(inner, -1.0, 1.0))
...
def _gripper_from_angular(value):
    value = _unnormalize(value, min_val=0.4, max_val=1.5)
    return _normalize(value, min_val=1.0, max_val=0.0)


def _gripper_from_angular_inv(value):
    value = _unnormalize(value, min_val=1.0, max_val=0.0)
    return _normalize(value, min_val=0.4, max_val=1.5)
...
def _decode_hsr(data: dict, *, adapt_to_pi: bool = False, convert_gripper: bool = False) -> dict:
    ...
    for key in ("head_rgb", "hand_rgb"):
        data[key] = convert_image(data[key])
    data["state"] = state
    return data


def _decode_state(state: np.ndarray, *, adapt_to_pi: bool = False, convert_gripper: bool = False) -> np.ndarray:
    if not adapt_to_pi:
        return state
...
def _decode_actions_inv(actions: np.ndarray, *, adapt_to_pi: bool = False) -> np.ndarray:
    if not adapt_to_pi:
        return actions

    aligned_ids = [0, 1, 2, 3, 4, 6, 11, 12, 13, 14, 15]
    return actions[:, aligned_ids]


def _encode_actions(actions: np.ndarray, *, adapt_to_pi: bool = False, convert_gripper: bool = False) -> np.ndarray:
    if adapt_to_pi and convert_gripper:
        actions[:, 5] = _gripper_from_angular(actions[:, 5])
    return actions


def _encode_actions_inv(actions: np.ndarray, *, adapt_to_pi: bool = False, convert_gripper: bool = False) -> np.ndarray:
    if adapt_to_pi and convert_gripper:
        actions[:, 6] = _gripper_from_angular_inv(actions[:, 6])
    return actions
